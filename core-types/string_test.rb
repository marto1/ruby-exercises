...
class StringTest < Minitest::Test
  ...
  def test_squeeze
    assert_equal "Grany jugles pupies in the monlight", 
                 "Granny juggles puppies in the moonlight".squeeze
    assert_equal "Sunny summer days playing hops on the rof", 
              "Sunny summer days playing hoops on the roof".squeeze('o')
    assert_equal "You fool! You canot choose to cook chese with a spoon",
    "You fool! You cannot choose to cook cheese with a spoon".squeeze('ne')
    assert_equal "Her name was Betty Moore",
    "Her name      was Betty Moore".squeeze(" ")
  end
  ...
  def test_format_specification
    ...
    assert_equal "Harry is 38 years old.",
             "%{name} is %{age} years old." % {name: "Harry", age: "38"}
    assert_equal "Harry is 38 years old.",
            "%{name} is %{age} years old." % {name: "Harry", age: "38"}
    assert_equal "10 years and 3.5 months",
   "%<years>d years and %<months>.1f months" % {years: 10, months: 3.478}
  end
